...
def get_current_weather(lat, lon):
    ...
    weather_data = one_call.forecast_hourly[0]

    return _get_weather_statements(weather_data)


def _get_weather_statements(data):
    time = datetime.datetime.fromtimestamp(data.ref_time).date().strftime(
        'Today is %B %d of %Y')

    temperature = f"The current temperature is " \
